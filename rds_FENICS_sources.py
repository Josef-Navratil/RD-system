...
f_2 = Expression('0.0*(exp(-0.25*pow(x[0]-11.0,2)-0.25*pow(x[1]-6.0,2))-0.0111)', degree=1)
f_3 = Expression('0.0*(exp(-0.25*pow(x[0]-3.0,2)-0.25*pow(x[1]-3.0,2))-0.0111)', degree=1)
        ...
        if k%10==0:
            ...
            if k%20==0:
                ...
                if no_of_files >= 100:
                    ...
                    file_location = "resultsSO-%.3f-%.3f-%s-%s"%(d1,d2,Kin,BC)+"/output%06d.vtu"%(no_of_files-100)
                    print(file_location)
...
error_file.write("H1 norm: {0}, L2 norm: {1}".format(norm(u,'H1'), norm(u))
error_file.close()
